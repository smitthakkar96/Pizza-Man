...
def send_welcome(message):
    ...
    if row.count() > 0:
        ...
        u = User(int(message.from_user.id), message.from_user.first_name +
                 " " + message.from_user.last_name, None, None, None, None)
        session.add(u)
        session.flush()
...
def send_image_and_confirmation(message):
    ...
    bot.reply_to(
        message,
        "Good choice you want the " +
        pizza["name"] +
        "\n shall I have this delivered to " +
        user.location,
        reply_markup=markup)
...
def finish(message):
    if message.text.lower() == "confirm":
        user = session.query(User).filter_by(
            id=int(message.from_user.id)).first().id
        text = message.text
        ...
        bot.reply_to(
            message,
            "Thank you sir your pizza will be delivered within 30 mins.")
    else:
        ...
        bot.reply_to(
            ...
            " " +
            message.from_user.last_name +
            " we have cancelled your order. Would you like to order something else ?",
            reply_markup=getPizzaMarkUp())
...
def reciveLocation(message):
    ...
    pdb.set_trace()
    user = session.query(User).filter_by(id=int(message.from_user.id)).first()
    if user.location == "Earth" or user.location is None:
        location_data = message.location
        location_decode = geolocator.reverse(
            str(location_data.latitude) + "," + str(location_data.longitude))
        ...
        itembtn2 = types.KeyboardButton(
            'Send Contact Number', request_contact=True)
        markup.add(itembtn2)
        bot.reply_to(
            message,
            "Can we have your contact number too ?",
            reply_markup=markup)
